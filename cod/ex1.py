...
img = cv2.imread('img/fundo.png', 0)
...
#E)
imagem = cv2.imread('img/fundo.png')
...
cv2.imwrite('img/fundo40.png', rotacionado)

plt.figure(figsize=(6.4*5, 4.8*5), constrained_layout=False)

#img rotacionada
img_rotacionada40 = cv2.imread('img/fundo40.png', 0)
...
#F)
#translação
deslocamento = np.float32([[1, 0, -55], [0, 1, -85]]) #parametros para movimentar a imagem 
deslocado = cv2.warpAffine(imagem, deslocamento, (largura, altura))
cv2.imshow("Cima e esquerda", deslocado)
cv2.waitKey(0)

cv2.imwrite('img/fundo_trans.png', deslocado)

plt.figure(figsize=(6.4*5, 4.8*5), constrained_layout=False)

#img transladada
img_trans = cv2.imread('img/fundo_trans.png', 0)
